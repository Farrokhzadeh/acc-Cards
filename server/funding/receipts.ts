  ...
  try {
    const result = await withTransaction(async (db)=>{
      ...
      const inserted=await db.query<{id:string}>(
        `INSERT INTO receipts(request_id,object_key,original_filename,mime_type,size_bytes,sha256_hex,scan_status,source,telegram_file_id,telegram_file_unique_id,detected_mime_type,scan_engine,scan_note,scan_completed_at)
         VALUES($1::uuid,$2,$3,$4,$5,$6,$7,'telegram',$8,$9,$10,$11,$12,now()) RETURNING id`,
        [input.requestId,stored.objectKey,stored.originalFilename,stored.declaredMimeType??stored.detectedMimeType,stored.sizeBytes,stored.sha256Hex,stored.scanStatus,input.telegramFileId,input.telegramFileUniqueId??file.file_unique_id,stored.detectedMimeType,stored.scanEngine,stored.scanNote],
      );
