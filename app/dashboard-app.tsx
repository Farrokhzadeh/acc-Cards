  ...
  return (
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <Card className={`surface-card rounded-[24px] ${settingsTab==="general"?"":"hidden"}`}><CardHeader><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-[13px] bg-[#e6f8f1] text-[#16815e]"><WalletCards className="size-5" /></span><div><CardTitle className="text-[17px] tracking-[-.02em] text-[#2c2940]">Card purchase minimum</CardTitle><p className="mt-1 text-sm text-[#8f8b9c]">Used when a customer buys their first card or requests an additional card.</p></div></div></CardHeader><CardContent className="space-y-4"><div className="grid gap-2"><Label htmlFor="pay-min-load">Minimum card purchase (USD)</Label><Input id="pay-min-load" type="number" min={1} value={firstCard.minLoadUsd} onChange={(e) => setFirstCard((c) => ({ ...c, minLoadUsd: Number(e.target.value) || 25 }))} className="bg-white" /><p className="text-xs text-[#8f8b9c]">The customer chooses an amount at or above this minimum for first and additional cards. The approved amount becomes that card&apos;s initial balance. Customers never choose a BIN.</p></div><Button className="rounded-xl bg-[#167957] text-white hover:bg-[#116144]" disabled={payBusy} onClick={() => void saveFirstCard()}><Check className="size-4" />Save card purchase minimum</Button></CardContent></Card>
