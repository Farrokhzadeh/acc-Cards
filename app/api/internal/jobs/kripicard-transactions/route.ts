...
export async function POST(request: Request) {
  return apiRoute(request, async ({ requestId }) => {
    ...
    if (!env.KRIPICARD_TRANSACTION_SYNC_JOB_SECRET) {
      throw new ApiError(503, "transaction_sync_job_disabled", "The Kripicard transaction sync hook is not configured.");
    }
    if (!authorized(request, env.KRIPICARD_TRANSACTION_SYNC_JOB_SECRET)) {
